...
class AdminController {
  ...
  static async getUsers(req: Request, res: Response) {
    ...
    if (membership_level) {
      where.membershipLevel = (membership_level as string).toUpperCase();
    }
  ...
  static async updateUser(req: Request, res: Response) {
    ...
    try {
      ...
      if (email_verified !== undefined) updateData.emailVerified = email_verified;
  ...
  static async getSystemStats(req: Request, res: Response) {
    try {
      const [
        ...
        prisma.user.count({ where: { membershipLevel: 'BASIC' } }),
        prisma.user.count({ where: { membershipLevel: 'PREMIUM' } }),
        prisma.user.count({ where: { membershipLevel: 'PROFESSIONAL' } }),
   ...
   * 获取用户会员使用统计
   */
  static async getUserMembershipStats(req: Request, res: Response) {
    const { userId } = req.params;
    ...
    try {
      ...
      const currentLimits = membershipFeatures[user.membershipLevel as keyof typeof membershipFeatures] || membershipFeatures.BASIC;
  ...
  static async batchUpdateUsers(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await prisma.user.updateMany({
        where: {
          id: { in: user_ids.map((id: string) => Number(id)) },
        },
        data: updateData,
      });
   ...
   * 获取会员级别统计
   */
  static async getMembershipAnalytics(req: Request, res: Response) {
    try {
      // 会员等级分布
      ...
      // 会员收入统计（模拟数据，实际应该从订单表计算）
      const revenueStats = {
        monthly_revenue: 2380, // $2,380
        yearly_revenue: 28560,  // $28,560
      ...
      // 即将到期的会员
      const expiringMemberships = await prisma.user.findMany({
        where: {
          membershipExpiresAt: {
   ...
   * 添加Claude账号
   */
  static async addClaudeAccount(req: Request, res: Response) {
    const { 
      account_name,
      api_key,
      organization_id,
   ...
   * 更新Claude账号
   */
  static async updateClaudeAccount(req: Request, res: Response) {
   ...
   * 删除Claude账号
   */
  static async deleteClaudeAccount(req: Request, res: Response) {
    const { accountId } = req.params;
   ...
   * 测试Claude账号连接
   */
  static async testClaudeAccount(req: Request, res: Response) {
    const { accountId } = req.params;
   ...
   * 获取Claude使用统计
   */
  static async getClaudeUsageStats(req: Request, res: Response) {
   ...
   * 获取代理服务器列表
   */
  static async getProxies(req: Request, res: Response) {
    try {
   ...
   * 获取智能调度配置
   */
  static async getSchedulerConfig(req: Request, res: Response) {
    try {
   ...
   * 更新智能调度配置
   */
  static async updateSchedulerConfig(req: Request, res: Response) {
   ...
   * 获取AI异常检测报告
   */
  static async getAIAnomalyDetection(req: Request, res: Response) {
    try {
