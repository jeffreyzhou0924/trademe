...
class EmailService {
   ...
   * 发送密码重置成功通知
   */
  public async sendPasswordResetNotification(email: string, username: string): Promise<boolean> {
    const template: EmailTemplate = {
   ...
   * 生成验证码邮件HTML模板
   */
  private getVerificationEmailHtml(code: string, action: string, description: string): string {
    return `
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
   ...
   * 生成密码重置成功通知HTML模板
   */
  private getPasswordResetNotificationHtml(username: string): string {
    return `
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
  ...
  public async close(): Promise<void> {
    try {
      ...
      logger.info('邮件服务连接已关闭');
    } catch (error) {
      logger.error('关闭邮件服务连接失败:', error);
    }
  }
