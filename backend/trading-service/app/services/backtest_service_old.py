...
# BacktestEngine 类已被 StatelessBacktestEngine 替代
# 旧的 BacktestEngine 类存在状态污染问题，已移除
    """回测引擎类"""
    ...
    async def _get_historical_data(
        ...
        try:
            ...
            # 🆕 使用数据验证服务检查数据可用性 - 智能产品类型映射
            def get_product_type_for_symbol(symbol: str) -> str:
                """根据symbol智能判断产品类型"""
                if 'SWAP' in symbol or symbol.endswith('-USDT-SWAP'):
                    return 'futures'  # OKX永续合约在DB中存储为futures
                elif 'PERP' in symbol:
                    return 'futures'
                else:
                    return 'spot'

            product_type = get_product_type_for_symbol(symbol)
            logger.info(f"🔧 智能产品类型映射: {symbol} → {product_type}")
    ...
    async def _execute_user_strategy_code(
        ...
        try:
            ...
            context.portfolio.cash = 10000.0
            context.portfolio.positions = {}

            # 传递context参数来实例化策略
            strategy_instance = UserStrategy(context)
...
class DeterministicBacktestEngine(BacktestEngine):
    ...
    async def _get_historical_data_deterministic(
        ...
        try:
            ...
            # 智能Symbol格式匹配：支持多种交易对格式
            def normalize_symbol_for_db_query(symbol: str) -> str:
                """智能转换symbol格式以匹配数据库中的实际格式"""
                # 如果是 BTC/USDT 格式，优先尝试 BTC-USDT-SWAP (OKX永续合约格式)
                if '/' in symbol:
                    base, quote = symbol.split('/')
                    if base in ['BTC', 'ETH', 'BNB', 'SOL'] and quote == 'USDT':
                        return f"{base}-{quote}-SWAP"
                    else:
                        return symbol.replace('/', '')
                return symbol

            normalized_symbol = normalize_symbol_for_db_query(symbol)
            logger.info(f"🔄 Symbol格式转换: {symbol} → {normalized_symbol}")
            ...
            for item in market_data:
                df_data.append({
                    'timestamp': pd.Timestamp(item.timestamp),  # 使用正确的字段名
