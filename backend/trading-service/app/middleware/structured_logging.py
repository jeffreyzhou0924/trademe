    ...
    def setup_logger(self):
        ...
        if settings.environment == "development":
            logger.add(
                ...
                serialize=True if settings.environment != "development" else False,
                backtrace=True,
                diagnose=True
            )
    ...
    def _get_log_format(self) -> str:
        ...
        if settings.environment == "development":
            ...
            return (
                ...
                "<white>{extra[request_id]}</white> | "
                "<level>{message}</level>"
            )
        else:
            # 生产环境使用JSON格式
            return "{time} | {level} | {name}:{function}:{line} | {extra[request_id]} | {message}"
